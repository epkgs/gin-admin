...
func (a *Menu) upsert(ctx context.Context, items model.Menus, parent *model.Menu) error {
	total := len(items)
	...
	for i, item := range items {
		...
		if item.Status == "" {
			item.Status = model.MenuStatus_ENABLED
		}
...
func (a *Menu) List(ctx context.Context, req dto.MenuListReq) (*dto.List[*model.Menu], error) {
	...
	scope := func(d gen.Dao) gen.Dao {
		...
		if v := req.Type; len(v) > 0 {
			...
			if v := req.WithResources; !v {
				d = d.Where(m.Type.Neq(model.MenuType_BUTTON))
			}
		}
...
func (a *Menu) appendChildren(ctx context.Context, data model.Menus) (model.Menus, error) {
	...
	appendData := func(child *model.Menu) {
...
func (a *Menu) Create(ctx context.Context, req *dto.MenuCreateReq) (*model.Menu, error) {
...
func (a *Menu) Update(ctx context.Context, id string, req *dto.MenuUpdateReq) error {
	...
	err = a.q.Transaction(func(tx *dao.Query) error {
		...
		if menu.Type != model.MenuType_MENU {
			_, err := m.WithContext(ctx).
				Where(m.ParentID.Eq(menu.ParentID)).
				Where(m.Type.Eq(model.MenuType_BUTTON)).
				Delete()
			if err != nil {
				return err
			}
...
func (a *Menu) delete(ctx context.Context, id string) error {
	...
	if err := m.Roles.Model(&model.Menu{ID: id}).Clear(); err != nil {
		return err
	}
